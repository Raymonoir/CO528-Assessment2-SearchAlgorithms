...
class GraphParser {
    public static GraphSearchProblem parse(String graphEncoding) {
        ...
        Set<GraphSearchState> goalStates = Arrays.asList(r[1].split("")).stream().map(GraphSearchState::new)
        ...
        for (int i = 2; i < lines.length; i++) {
            ...
            Collection<SuccessorInfo<GraphSearchState, GraphAction>> successorsList = successors.containsKey(fromNode)
                    ? successors.get(fromNode)
                    : new ArrayList<>();
        ...
        for (GraphSearchState state : allStates) {
            Collection<SuccessorInfo<GraphSearchState, GraphAction>> successorsList = successors.containsKey(state)
...
class GraphSearchTest<S, A> extends TestCase<S, A> {
    ...
    public TestResult execute() throws Exception {
        ...
        SearchHeuristic<GraphSearchState, GraphAction> heuristic = heuristicEncoding == null
                ? new NullHeuristic<GraphSearchState, GraphAction>()
                : GraphParser.parseHeuristic(heuristicEncoding);
        ...
        String correctSolutionMessage = "\tcorrect solution:\t\t" + goldSolutions.get(0) + "\n" +
                "\tcorrect expanded states:\t" + expandedSets.get(0) + "\n";
        if (expandedSets.size() > 1 && goldSolutions.size() > 1) {
            correctSolutionMessage += "\tcorrect reverse solution:\t\t" + goldSolutions.get(1) + "\n" +
...
abstract class PacmanTestCase<S, A> extends TestCase<S, A> {
    ...
    public ProblemSetup<S, A> setupProblem(String heuristicName) throws Exception {
        ...
        SearchProblem<S, A> searchProblem = (SearchProblem) Class.forName(this.searchProblemClassName)
...
class PacmanCornersTest extends PacmanTestCase<PacmanCornersSearchState, PacmanAction> {
    ...
    public TestResult execute() throws Exception {
        ...
        Solution<PacmanCornersSearchState, PacmanAction> solution = GraphSearch.search(strategy, setup.problem,
                setup.heuristic);
        ...
        if (notVisitedCorners.contains(coordinate))
            notVisitedCorners.remove(coordinate);
...
class GradedHeuristicTest<S, A> extends PacmanTestCase<S, A> {
    ...
    public TestResult execute() throws Exception {
        ...
        return new TestResult(true,
                pass + ": Heuristic resulted in expansion of " + setup.problem.getExpandedCount() + " nodes\n");

    }
}

class ProblemEncodings {
    static final String GRAPH_BACKTRACK = "start_state: A\n" +
            "goal_states: G\n" +
            "A 0:A->B B 1.0\n" +
            "A 1:A->C C 2.0\n" +
            "A 2:A->D D 4.0\n" +
    ...
    static final String GRAPH_BFS_VS_DFS = "start_state: A\n" +
            "goal_states: G\n" +
            "A 0:A->B B 1.0\n" +
            "A 1:A->G G 2.0\n" +
            "A 2:A->D D 4.0\n" +
    ...
    static final String GRAPH_INFINITE = "start_state: A\n" +
            "goal_states: G\n" +
            "A 0:A->B B 1.0\n" +
            "B 0:B->A A 2.0\n" +
            "B 1:B->C C 4.0\n" +
    ...
    static final String GRAPH_MANY_PATHS = "start_state: A\n" +
            "goal_states: G\n" +
            "A 0:A->B1 B1 1.0\n" +
            "A 1:A->C C 2.0\n" +
            "A 2:A->B2 B2 4.0\n" +
    ...
    static final String GRAPH_A_STAR_0 = "start_state: A\n" +
            "goal_states: H F\n" +
            "A Right B 2.0\n" +
            "B Right H 4.0\n" +
            "B Down D 1.0\n" +
    ...
    static final String GRAPH_HEURISTIC = "start_state: S\n" +
            "goal_states: G\n" +
            "S 0 A 2.0\n" +
            "S 1 B 3.0\n" +
            "S 2 D 5.0\n" +
    ...
    static final String GRAPH_GOAL_AT_DEQUEUE = "start_state: A\n" +
            "goal_states: G\n" +
            "A 0:A->G G 10.0\n" +
            "A 1:A->B B 1.0\n" +
            "B 0:B->C C 1.0\n" +
    ...
    static final List<PacmanAction> MEDIUM_MAZE_SOLUTION_DFS1 = List.of(PacmanAction.WEST, PacmanAction.WEST,
            ...
            PacmanAction.WEST, PacmanAction.WEST, PacmanAction.WEST, PacmanAction.WEST, PacmanAction.WEST);
    static final List<PacmanAction> MEDIUM_MAZE_SOLUTION_DFS2 = List.of(PacmanAction.SOUTH, PacmanAction.SOUTH,
            PacmanAction.WEST, PacmanAction.WEST, PacmanAction.WEST,
            ...
            PacmanAction.WEST, PacmanAction.WEST, PacmanAction.WEST, PacmanAction.WEST, PacmanAction.WEST,
            PacmanAction.WEST);
    static final List<PacmanAction> MEDIUM_MAZE_SOLUTION_BFS = List.of(PacmanAction.WEST, PacmanAction.WEST,
            PacmanAction.WEST, PacmanAction.WEST, PacmanAction.WEST,
            PacmanAction.WEST, PacmanAction.WEST, PacmanAction.WEST, PacmanAction.WEST, PacmanAction.SOUTH,
...
public class Autograder {
    ...
    static final QuestionSuite Q1 = new QuestionSuite("Q1", new TestCase[] {
            ...
            new GraphSearchTest<GraphSearchState, GraphAction>(
                    ...
                    "GraphSearchProblem", "dfs",
                    List.of("2:A->B2 0:B2->C 0:C->D 2:D->E2 0:E2->F 0:F->G",
                            "0:A->B1 0:B1->C 0:C->D 0:D->E1 0:E1->F 0:F->G"),
